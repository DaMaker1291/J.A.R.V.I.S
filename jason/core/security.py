...
class AegisManager:
    ...
    def _run_sudo_command(self, cmd: List[str], timeout: int = 5) -> bool:
        """Run a command with sudo, checking for root privileges first"""
        if os.geteuid() != 0:
            logger.warning(f"Root privileges required for: {' '.join(cmd)}. Skipping.")
            return False
        try:
            subprocess.run(['sudo'] + cmd, check=True, timeout=timeout)
            return True
        except Exception as e:
    ...
    def _initialize_firewall(self):
        ...
        try:
            # Check if PF is available (macOS)
            if os.uname().sysname != 'Darwin':
                logger.warning("Aegis Firewall currently only supports macOS (PF)")
                return
    ...
    def _enable_pf(self):
        ...
        if os.geteuid() != 0:
            logger.warning("Root privileges required to enable PF. Skipping auto-enable.")
            return

        try:
            # Enable PF
            subprocess.run(['sudo', 'pfctl', '-e'], check=True, timeout=5)
